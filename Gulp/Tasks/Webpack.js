...
module.exports = function(config) {
					...
					res.on('end', () => {
						try {
							...
							for (let tunnel of parsed.tunnels) {
								switch (tunnel.name) {
									case 'gc-backend (http)':
										GC_TUNNELS.backend = tunnel.public_url;
										break;

									case 'gc-frontend (http)':
										GC_TUNNELS.frontend = tunnel.public_url;
										break;
								}
							}
							resolve(GC_TUNNELS);
		...
		webpackSectionConfigs[section] = {
			...
			plugins: [
				...
				new webpack.DefinePlugin({
					GC_TUNNELS: JSON.stringify({}),
				}),
				new webpack.DefinePlugin({
					GC_SECTION: JSON.stringify(section),
					GC_ENVIRONMENT: JSON.stringify(
						!config.developmentEnv ? 'production' : 'development'
					),
					GC_BUILD_TYPE: JSON.stringify(config.production ? 'production' : 'development'),
					GC_IS_CLIENT: JSON.stringify(!!config.client),
					GC_IS_SSR: JSON.stringify(config.ssr === 'server'),
					GC_VERSION: JSON.stringify(
						require(path.resolve(process.cwd(), 'package.json')).version
					),
					GC_MANIFEST_URL: JSON.stringify(
						require(path.resolve(process.cwd(), 'package.json')).clientManifestUrl
					),
					GC_WITH_UPDATER: JSON.stringify(
						(!config.developmentEnv && !config.watching) || config.withUpdater
					),
					GC_IS_WATCHING: JSON.stringify(config.watching),
				}),
				...
				hasOfflineSupport
					? new OfflinePlugin({
							...
							ServiceWorker: {
								events: true,
								output: 'sjw.js',
								publicPath: 'https://gamingchannel.com/sjw.js',
							},
					  })
					: noop,
	...
	gulp.task(
		...
		gulp.series('clean', function(cb) {
			...
			getTunnels
				.then(GC_TUNNELS => {
					console.log(GC_TUNNELS);
					for (let buildSection of buildSections) {
						// Insert another define plugin with the GC_TUNNELS const.
						const sectionConfig = webpackSectionConfigs[buildSection];
						sectionConfig.plugins.splice(
							2, // replace the default GC_TUNNELS define plugin.
							1,
							new webpack.DefinePlugin({
								GC_TUNNELS: JSON.stringify(GC_TUNNELS),
							})
						);
						...
						let server = new WebpackDevServer(compiler, {
							...
							public: 'development.gamingchannel.com',
							quiet: true,

...
export default class AppAuthLogin extends Vue {
	...
	onLoggedIn() {
		if (this.redirectTo) {
			...
			// Subdomain redirect: jams.GamingChannel.io, fireside.GamingChannel.com, etc.
			// This also handles main domain.
			if (this.redirectTo.search(/^https?:\/\/([a-zA-Z.]+\.)?GamingChannel.(com|io)/) !== -1) {
				Navigate.goto(this.redirectTo);
				return;
			}

			// Normal redirect, within the GamingChannel.com domain.

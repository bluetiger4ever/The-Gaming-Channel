...
export abstract class ClientUpdater {
	...
	static async checkForClientUpdates() {
		...
		try {
			...
			try {
				...
				// begun and attempt to check for updates again. Coltron responds with the following error message.
				//
				// This should not be possible because at the time of initializing the updater service
				// it checks the current state of coltron before attempting to check for updates and would skip
				// the check if its already in progress. Something weird is going on.
				//
				// Either way, this is an error we should be able to tolerate and just try syncing with coltron's state again.
	...
	static async updateClient() {
		...
		try {
			console.log('updateClient in clientLibrary store: ' + JSON.stringify(GC_WITH_UPDATER));

			const updaterInstance = await this.ensureUpdaterInstance();

			if (this.clientUpdateStatus !== 'ready') {
				throw new Error('Failed to apply the update. Coltron is not in a ready state');
			}
	...
	private static async createUpdaterInstance() {
		...
		const builder = new Promise<SelfUpdaterInstance>(async (resolve, reject) => {
			...
			try {
				const manifestPath = path.resolve(Client.coltronDir, '.manifest');
	...
	// Queries the current state of coltron. We might already by in the middle of an update,
	...
	private static async queryUpdaterState(instance: SelfUpdaterInstance) {
		console.log('Checking current status of coltron...');
		const state = await instance.controller.sendGetState(false, 5000);
		console.log('Current coltron status: ');
		console.log(state);
